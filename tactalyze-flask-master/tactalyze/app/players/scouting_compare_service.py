...
def empty_rows(df1, df2):
    a = np.zeros(len(df1.columns))
    a = a.tolist()
    diff = len(df1) - len(df2)
    start_date = df2['Date'].iloc[-1]
...
def run(filestream1, filestream2, dates, end_date, dir):
    ...
    if len(stats_per90_player2) > len(stats_per90_player1):
        stats_per90_player1 = empty_rows(stats_per90_player2, stats_per90_player1)
    elif len(stats_per90_player1) > len(stats_per90_player2):
        stats_per90_player2 = empty_rows(stats_per90_player1, stats_per90_player2)
    ...
    try:
        ...
        end_index = nearest_ind(stats_per90_player1['Date'], datetime.date(end_date.year, end_date.month, end_date.day))
    except ValueError:
        dates = None
    ...
    if year_frame == 3:
        season_index_player1 = nearest_ind(stats_per90_player1['Date'], datetime.date(current_date.year - 3, 6, 1))
        season_index1_player1 = nearest_ind(stats_per90_player1['Date'], datetime.date(current_date.year - 2, 6, 1))
        season_index2_player1 = nearest_ind(stats_per90_player1['Date'], datetime.date(current_date.year - 1, 6, 1))
        season_index3_player1 = nearest_ind(stats_per90_player1['Date'], datetime.date(current_date.year, 6, 1))
        ...
        season_index5_player1 = 0

        season_index_player2 = nearest_ind(stats_per90_player2['Date'], datetime.date(current_date.year - 3, 6, 1))
        season_index1_player2 = nearest_ind(stats_per90_player1['Date'], datetime.date(current_date.year - 2, 6, 1))
        season_index2_player2 = nearest_ind(stats_per90_player1['Date'], datetime.date(current_date.year - 1, 6, 1))
        season_index3_player2 = nearest_ind(stats_per90_player1['Date'], datetime.date(current_date.year, 6, 1))
        season_index4_player2 = nearest_ind(stats_per90_player2['Date'],
                                        datetime.date(current_date.year, current_date.month, current_date.day))
        season_index5_player2 = 0
        ...
        season_ticks3 = [str((current_date.year - 3) % 100) + '/' + str((current_date.year - 2) % 100),
                     ...
                     str((current_date.year - 1) % 100) + '/' + str(current_date.year % 100),
                     str((current_date.year) % 100) + '/' + str((current_date.year + 1) % 100)]
        season_ticks4 = []


    elif year_frame == 4:
        season_index_player1 = nearest_ind(stats_per90_player1['Date'], datetime.date(current_date.year - 4, 6, 1))
        season_index1_player1 = nearest_ind(stats_per90_player1['Date'], datetime.date(current_date.year - 3, 6, 1))
        season_index2_player1 = nearest_ind(stats_per90_player1['Date'], datetime.date(current_date.year - 2, 6, 1))
        season_index3_player1 = nearest_ind(stats_per90_player1['Date'], datetime.date(current_date.year - 1, 6, 1))
        season_index4_player1 = nearest_ind(stats_per90_player1['Date'], datetime.date(current_date.year, 6, 1))
        season_index5_player1 = nearest_ind(stats_per90_player1['Date'],
                                datetime.date(current_date.year, current_date.month, current_date.day))

        season_index_player2 = nearest_ind(stats_per90_player2['Date'], datetime.date(current_date.year - 4, 6, 1))
        season_index1_player2 = nearest_ind(stats_per90_player2['Date'], datetime.date(current_date.year - 3, 6, 1))
        season_index2_player2 = nearest_ind(stats_per90_player2['Date'], datetime.date(current_date.year - 2, 6, 1))
        season_index3_player2 = nearest_ind(stats_per90_player2['Date'], datetime.date(current_date.year - 1, 6, 1))
        season_index4_player2 = nearest_ind(stats_per90_player2['Date'], datetime.date(current_date.year, 6, 1))
        season_index5_player2 = nearest_ind(stats_per90_player2['Date'],
                                datetime.date(current_date.year, current_date.month, current_date.day))
    ...
    for column in list(stats_per90_player1.columns[9:]):
        try:
            ...
            if dates is not None:
                ...
                ax.axvline(x=end_index, color='#E23D46', linewidth=2, label='End Tactalyse')
            ...
            if year_frame == 3:
                ...
                ax.axvline(x=season_index3_player1, color='gray', linestyle='--')  # marks end of current_year
            ...
            if year_frame == 4:
                ...
                ax.axvline(x=season_index4_player1, color='gray', linestyle='--')  # marks end of current_year
            ...
            elif column == 'Offensive duels' or column == 'Defensive duels' or column == 'Aerial duels' or column == 'Duels' or column == 'Loose ball duels':
                plt.title(plot_name + '\n' + column + ' per 90 minutes')
            ...
            elif column == 'Interceptions' or column == 'Clearances' or column == 'Yellow cards' or column == 'Red cards' or column == 'Fouls' or \
                    column == 'Shot assists' or column == 'Fouls suffered' or column == 'Received passes' or column == 'Exits':
                plt.title(plot_name + '\n' + column + ' per 90 minutes')
            ...
            # 2019, 2020, 2021
            if year_frame == 3:
                plt.xticks([(season_index_player1 + season_index1_player1) / 2, (season_index2_player1 + season_index1_player1) / 2,
                            (season_index2_player1 + season_index3_player1) / 2, (season_index3_player1 + season_index4_player1) / 2], season_ticks3)
            ...
            if path[0] == '/':
                fig_name = dir + '/' + column + '_' + str(window) + '.png'
            else:
                fig_name = dir + '\\' + column + ' ' + str(window) + '.png'
            ...
            print(type(stats_per90_player1))
