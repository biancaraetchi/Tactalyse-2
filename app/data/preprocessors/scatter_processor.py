...
class ScatterProcessor(Preprocessor):
    ...
    def extract_scatter_data(self, player_file, compare_file, player_name, compare_name):
        ...
        :param compare_name:
        :return: a map with dataframes for the player (`player_data`) and the second player, if any was provided,
        (`compare_player_data`) their names (`player_name` and `compare_player_name`, respectively) and the columns that
        need to be selected from the dataframe (`columns`, in the following format [[x1, y1], [x2, y2], ...]).
        """
        reader = ExcelReader()
        scatter_map = {}
    ...
    def set_columns(self, scatter_map):
        ...
        if isinstance(scatter_map.get("compare_player_data"), type(None)):
            ...
            return scatter_map
        columns = self.drop_unique_columns(scatter_map.get("player_data"), scatter_map.get("compare_player_data"),
                                           columns_player)
