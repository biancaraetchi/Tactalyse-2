...
class ScatterPlot(Graph):
    ...
    def draw(self, param_map):
        ...
        fig, ax = plt.subplots()
        ax.clear()
        # axis_titles needs to be split on the '/', since the data columns are formatted in the following manner:
        # "Total goals / successful", where "Total goals" and "successful" will represent different axis names
        ...
        if isinstance(compare_data, type(None)):
            ...
            enter_data = {axis_titles[0]: data[data.columns[column_numbers[0]]],
                          axis_titles[1]: data[data.columns[column_numbers[1]]]}
            hue = None
        else:
            # if there is a second player, add a column to the dataframes, each representing the name of the respective
            # player (for plot legend purposes)
            ...
            enter_data = {axis_titles[0]: necessary_data[data.columns[column_numbers[0]]],
                          axis_titles[1]: necessary_data[data.columns[column_numbers[1]]],
                          "Player": necessary_data['Player']}
            hue = "Player"
    ...
    def draw_all(self, param_map):
        ...
        :param param_map: a map with dataframes for the player (`player_data`) and the second player, if any was
        provided, (`compare_player_data`) their names (`player_name` and `compare_player_name`, respectively) and the
        columns that need to be selected from the dataframe (`columns`, in the following format [[x1, y1], [x2, y2],
        ...]).
        ...
        for column in columns:
            # rewrite the `columns` variable to represent a single pair of columns at a time (ease of use in next
            # function).
