...
The code functions as a backend API for generating Tactalyse's football player PDF reports. It was written  
in Python, and uses the Flask library as its web framework. Aside from a controller, the app has three distinct  
modules that are used to process the input data into a PDF report:

#### Data Preprocessing

The `data` module contains all functionality related to extracting data from Excel files into Pandas  
...
converting date strings to integer values for line plots. This is part of the "View" component.

#### PDF Generation

The `pdf_generator` module contains all functionality related to placing data and graphs received from the  
`controller` onto pages of a PDF file using fpdf2, and outputting it in bytestring. It is mostly concerned with  
layout of existing elements, such as graphs and images, on pages using pixel values. It comprises the second  
half of the "View" component.
...
- `league-name`: String representing the name of the football league for which data is contained in  
`league-file`. If not passed, it defaults to "Unknown".
- `compare-name`: String representing the name of the player to compare to in the PDF report's graphs. If this  
is passed, `compare-file` is expected as well.
...
format.
- `player-image`: Image to be used for the main player in the introductory page of the report. Defaults to  
`Default.png` from the resources folder in the `pdf_generator` module.
- `compare-image`: Image to be used for the comparison player in the introductory page of the report. Defaults  
to `Default.png` from the resources folder in the `pdf_generator` module.

## Data Formatting

As mentioned, the input data for the reports comes from Excel files. These Excel files are obtained from  
[Wyscout](https://wyscout.com/), a professional platform for football statistics and data. We make a distinction  
between two types of data files:

#### Player Files

Player files are Excel files that contain match data for a single player. Every row consists of data from a  
single match. Examples of these files can be found in `app/pdf_generator/resources/test_data`. The files are  
named "Player stats [...]". To avoid incompatibility with the current code, ensure that the passed `player-file`  
and `compare-file` are both in the same format as these files, i.e. contain the same column headers. The used  
columns per player position can be found in the "Stats per position" file from the same test_data folder. These  
can be updated to fit with changes in column names.
...
both `player-name` and `compare-name` are included in this column for the passed `league-file`. The same rules  
apply for these files as for the player files regarding formatting. An example of a league file can once again  
be found in the earlier-described test_data folder, by the name of "ENG2". The used columns per position can be  
found in `app/data/preprocessors/preprocessor`, in the `league_category_dictionary()` function. These can be  
updated to fit with changes in column names.
...
essentially the Flask app itself, and contains all endpoints. It also contains `service` files, which act as  
the communication channels between the app, and their dedicated modules. The `Service` classes were designed to  
return parameter maps, to improve code extendability (new endpoint parameters only need to be added to the map  
instead of every function signature).
...
each dedicated graph, and the PDF. The only data processing that is not contained in these classes is processing  
to do with converting values from their literal representations to representations that can be used in plots,  
as exemplified in [About the Application](#about-the-application).
...
After graphs have been generated, we would like to present them in a comprehensive PDF file, as a data report.  
This is done with the `pdf_generator` module. The main `PDF` class contains functions for placing elements on  
pages of a PDF document, and is an extension of the `FPDF` class from the fpdf2 library. These functions are  
then used by `PDFGenerator` classes, contained in the `generators` subdirectory. Currently, one generator  
exists for both a standard single player report, and a comparison report. A `Player` class has been added to  
contain information about players represented in the report, to avoid a long list of `PDF` class attributes.  
The `pdf_generator` module also contains the default data resources used in the PDF generation process, such  
as images and logos, as well as example data.
...
as it contains all functionality to do with handling data for non-visual purposes. The `graph_generator` and  
`pdf_generator` modules comprise the View component, as they generate visual output based on input data.
...
As of current, the `app.py` file contains functions that are not strictly endpoints. Rather, they deal with  
handling the incoming request in different formats (json, key-value form). In the Tactalyse Graph API that was  
developed in parallel with this API, we already refactored these functions into `Service` classes, and turned  
what are `Service` classes in this API into `Communicator` classes. A similar change could be made for this API,  
to keep the code decoupled and easily maintainable.
